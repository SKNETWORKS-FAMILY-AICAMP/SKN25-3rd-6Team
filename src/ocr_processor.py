    ...
    for filename in os.listdir(pdf_folder):
        if filename.lower().endswith(".pdf"):
            ...
            try:
                ...
                print(f"[ERROR] OCR 처리 중 오류: {filename} - {e}")
                continue
...
if __name__ == "__main__":
    ...
    OCR_OUTPUT_PATH = os.path.join(BASE_DIR, "data", "ocr_output")

    os.makedirs(OCR_OUTPUT_PATH, exist_ok=True)

    # 모든 카드사 폴더 순회
    card_companies = [d for d in os.listdir(RAW_DATA_PATH) if os.path.isdir(os.path.join(RAW_DATA_PATH, d))]
    
    print(f"\n{'='*80}")
    print(f"[START] 전체 카드사 OCR 처리 시작")
    print(f"카드사 수: {len(card_companies)}")
    print(f"{'='*80}\n")

    total_files = 0
    for card_company in sorted(card_companies):
        company_folder = os.path.join(RAW_DATA_PATH, card_company)
        pdf_count = len([f for f in os.listdir(company_folder) if f.lower().endswith(".pdf")])
        
        if pdf_count == 0:
            print(f"[SKIP] {card_company}: PDF 없음")
            continue

        print(f"\n[{card_company.upper()}] - {pdf_count}개 PDF")
        print("-" * 80)
        
        save_ocr_pdfs_to_txt(company_folder, OCR_OUTPUT_PATH)
        total_files += pdf_count

    print(f"\n{'='*80}")
    print(f"[COMPLETE] OCR 처리 완료!")
    print(f"총 처리된 PDF: {total_files}개")
    print(f"추출 위치: {OCR_OUTPUT_PATH}")
    print(f"{'='*80}\n")